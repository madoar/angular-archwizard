...
- a destination **step index**:
  ...
  ```html
  <button [awGoToStep]="{ stepIndex: 2 }" (finalize)="finalizeStep()">
    Go directly to the third Step
  </button>
  ```
...
- a destination **step id**:
  ...
  ```html
  <button
    [awGoToStep]="{ stepId: 'unique id of the third step' }"
...
- a **step offset** between the current step and the destination step:
  ...
  ```html
  <button [awGoToStep]="{ stepOffset: 1 }" (finalize)="finalizeStep()">
